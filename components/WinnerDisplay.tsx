...
const WinnerDisplay: React.FC<WinnerDisplayProps> = ({ winner, onReset, onClose }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[100] p-4">
      <div className="bg-slate-800 rounded-xl shadow-2xl p-8 max-w-lg w-full text-center border-2 border-amber-400 transform transition-all animate-fade-in-up">
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-4">
            ...
            <button
              onClick={onReset}
              className="w-full flex items-center justify-center gap-2 bg-amber-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-amber-700 transition-all duration-200"
            >
